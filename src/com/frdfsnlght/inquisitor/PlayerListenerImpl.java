...
*/
    ...
    public void onPlayerChatAsync(final AsyncPlayerChatEvent event) {
        if (event.isAsynchronous())
            Utils.fire(new Runnable() {
                @Override
                public void run() {
                    if (! PlayerStats.isStatsPlayer(event.getPlayer())) return;
    ...
    public void onPlayerDropItem(PlayerDropItemEvent event) {
        ...
        stats.add("itemsDropped", Utils.titleCase(event.getItemDrop().getItemStack().getType().name()), amount);
    }
    ...
    public void onPlayerPickupItem(PlayerPickupItemEvent event) {
        ...
        stats.add("itemsPickedUp", Utils.titleCase(event.getItem().getItemStack().getType().name()), amount);
    }
        ...
        switch (event.getPlayer().getItemInHand().getType()) {
            case BUCKET:
                if (event.getRightClicked() instanceof Cow)
                    PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("cowsMilked");
                break;
            case BOWL:
                if (event.getRightClicked() instanceof MushroomCow)
                    PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("mooshroomsMilked");
                break;
            case INK_SACK:
                if (event.getRightClicked() instanceof Sheep)
                    PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("sheepDyed");
                break;
        }
    }
    ...
    public void onPlayerExp(PlayerExpChangeEvent event) {
        if (! PlayerStats.isStatsPlayer(event.getPlayer())) return;
        PlayerStats.group.getStatistics(event.getPlayer().getName()).add("lifetimeExperience", event.getAmount());
    }

    @EventHandler(priority=EventPriority.MONITOR, ignoreCancelled = true)
    public void onEnchant(EnchantItemEvent event) {
        if (! PlayerStats.isStatsPlayer(event.getEnchanter())) return;
        PlayerStats.group.getStatistics(event.getEnchanter().getName()).incr("itemsEnchanted");
        PlayerStats.group.getStatistics(event.getEnchanter().getName()).add("itemEnchantmentLevels", event.getExpLevelCost());
    }
    ...
    public void onPlayerShearEntity(PlayerShearEntityEvent event) {
        ...
        switch (event.getEntity().getType()) {
            case SHEEP:
                PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("sheepSheared");
                break;
            case MUSHROOM_COW:
                PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("mooshroomSheared");
                break;
        }
    }
    ...
    public void onPlayerFishEvent(PlayerFishEvent event) {
        ...
        if (! PlayerStats.isStatsPlayer(event.getPlayer())) return;
        PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("fishCaught");
    }

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onPlayerEggThrow(PlayerEggThrowEvent event) {
        if (! PlayerStats.isStatsPlayer(event.getPlayer())) return;
        PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("eggsThrown", Utils.normalizeEntityTypeName(event.getHatchingType()));
    }
    ...
    public void onPlayerBucketEmpty(PlayerBucketEmptyEvent event){
        ...
        switch (bucket) {
            case WATER_BUCKET:
                PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("waterBucketsEmptied");
                break;
            case LAVA_BUCKET:
                PlayerStats.group.getStatistics(event.getPlayer().getName()).incr("lavaBucketsEmptied");
                break;
        }
    }
    ...
    public void onCraftItem(CraftItemEvent event){
        ...
        if (! PlayerStats.isStatsPlayer((Player)event.getWhoClicked())) return;
        Statistics stats = PlayerStats.group.getStatistics(((Player)event.getWhoClicked()).getName());
